...
date: "2024-06-26"
---
## data input and install packages
...
```{r give an id to each paper, echo=FALSE}
all_exp <- all_exp %>%
  group_by(Title) %>%
...
## Section 3.1 Orgnism/Contaminant types and publish year
...
# 显示桑基图
sankey
num_organisms <- n_distinct(all_exp$Organism)
num_organisms
# 查看每种 Organism 类型的数量
organism_counts <- all_exp %>%
  count(Organism)
organism_counts
...
```{r Suspending_medium、Suspending & Organism_type}
library(dplyr)
library(networkD3)
...
suspending_data <- all_exp %>%
 ...
 filter(!is.na(Suspending_medium) & Suspending_medium != "")
...
```
# 3.2.2 Donor surface/hand contamination method
...
all_exp <- all_exp %>%
  mutate(DryingTimeGroup = case_when(
    `Pre-contact_Preparation (wet or dry)` == "Dry" & `drying duration` == "24h" ~ "24 hours",
...
```{r Group bar chart of contaminate method in different color}
# Update the drying duration to include all surface conditions and unique categories
all_exp <- all_exp %>%
  mutate(DryingTimeGroup = case_when(
    `Pre-contact_Preparation (wet or dry)` == "Dry" & `drying duration` == "24h" ~ "Dry: 24 hours",
    `Pre-contact_Preparation (wet or dry)` == "Dry" & `drying duration` >= 1 & `drying duration` <= 15 ~ "Dry: 1-15 minutes",
    `Pre-contact_Preparation (wet or dry)` == "Dry" & `drying duration` > 15 & `drying duration` <= 30 ~ "Dry: 16-30 minutes",
    `Pre-contact_Preparation (wet or dry)` == "Dry" & `drying duration` > 30 & `drying duration` <= 45 ~ "Dry: 31-45 minutes",
    `Pre-contact_Preparation (wet or dry)` == "Dry" & `drying duration` > 45 & `drying duration` <= 60 ~ "Dry: 46-60 minutes",
    `Pre-contact_Preparation (wet or dry)` == "Dry" ~ "Dry: until dry", # Handling for dry but missing duration
    `Pre-contact_Preparation (wet or dry)` == "Frozen" ~ "Frozen",
    `Pre-contact_Preparation (wet or dry)` == "Wet" ~ "Wet",
    `Pre-contact_Preparation (wet or dry)` == "Not mentioned" ~ "Not mentioned",
    TRUE ~ "Other"  # For any unexpected cases
  )) %>%
  # Set factor levels to define legend order
  mutate(DryingTimeGroup = factor(DryingTimeGroup, levels = c(
    "Dry: 1-15 minutes", "Dry: 16-30 minutes", "Dry: 31-45 minutes", "Dry: 46-60 minutes", 
    "Dry: 24 hours", "Dry: until dry", "Frozen", "Wet", "Not mentioned", "Other"
  )))
...
custom_colors <- c(
  "Dry: 1-15 minutes" = "#66c2a5",
  "Dry: 16-30 minutes" = "#fc8d62",
  "Dry: 31-45 minutes" = "#8da0cb",
  "Dry: 46-60 minutes" = "#e78ac3",
  "Dry: 24 hours" = "#a6d854",
  "Dry: until dry" = "#ffd92f",
  "Frozen" = "#e5c494",
  "Wet" = "#b3b3b3",
  "Not mentioned" = "#1f78b4",
  "Other" = "#d73027"
)
...
ggplot(summary_data, aes(x = `Pre-contact_Preparation (wet or dry)`, y = UniqueID_Count, fill = DryingTimeGroup)) +
  ...
  labs(
    ...
    fill = "Drying Time Group / Surface Condition"
  ) +
  theme_minimal() +  # Use minimal theme as a base
  theme(
    panel.grid = element_blank(),         # Remove all grid lines
...
# 3.2.3 Contact behaviour

```{r contact duration}
# Ensure 'time' is numeric
contact_exp$time <- as.numeric(as.character(contact_exp$time))

# Plot histogram by Donor_surface_type
ggplot(contact_exp, aes(x = time, fill = `Pre-contact_Preparation (Wet or Dry)`)) + 
  geom_histogram(binwidth = 1, color = "black", alpha = 0.7, position = "stack") +
  labs(#title = "Frequency of Time in Contact Experiments by Surface condition", 
       x = "Time (seconds)", 
       y = "Frequency", 
 ...
 # theme_minimal() +
  ...
  scale_fill_brewer(palette = "Set3")  # Use a color palett
```
## Section 3.3 Transfer direction and surface materials

# 3.3.1 Transfer direction: donor and recipient surfaces
...
# Create donor and recipient nodes with internal IDs and display labels
nodes <- data.frame(
  name_display = c(unique(filtered_data$Donor_surface_type), unique(filtered_data$Recipient_surface_type)), # Labels without prefixes
  name_internal = c(paste0("Donor_", unique(filtered_data$Donor_surface_type)),
                    paste0("Recipient_", unique(filtered_data$Recipient_surface_type))) # Unique internal names
)

# Assign groups to nodes for coloring
nodes <- nodes %>%
  mutate(
    group = ifelse(grepl("^Donor_", name_internal), "Donor", "Recipient")
  )

# Create links with correct source and target IDs
links <- filtered_data %>%
  group_by(Donor_surface_type, Recipient_surface_type) %>%
  summarise(value = n(), .groups = "drop") %>%
  mutate(
    source = match(paste0("Donor_", Donor_surface_type), nodes$name_internal) - 1,
    target = match(paste0("Recipient_", Recipient_surface_type), nodes$name_internal) - 1
  )

# Optional: Create a custom color scale for nodes
color_scale <- 'd3.scaleOrdinal()
                .domain(["Donor", "Recipient"])
                .range(["#1f77b4", "#ff7f0e"])'  # Blue for Donor, Orange for Recipient
...
sankey <- sankeyNetwork(
  ...
  Target = "target",
  Value = "value",
  NodeID = "name_display",  # Use labels without prefixes
  NodeGroup = "group",      # Use groups for node coloring
...
# 3.3.2 Materials: fomite surface type, hand type
```{r Donor_surface_type}
filtered_data <- all_exp[all_exp$Donor_surface_type != "hand", ]

# 去掉 Donor_surface_type 为 'hand' 的行
filtered_data_no_hand <- filtered_data %>%
  filter(Donor_surface_type != 'Hand')

# 计算每个 Donor_surface_type 的唯一文章数
article_counts <- filtered_data_no_hand %>%
  group_by(Donor_surface_type) %>%
  summarise(ArticleCount = n_distinct(id))

# 绘制按文章数量分组的条形图
ggplot(article_counts, aes(x = Donor_surface_type, y = ArticleCount
)) +
  geom_bar(stat = "identity") +
  xlab("Donor Surface Type") +
  ylab("Number of Articles") +
  ggtitle("Number of Articles by Donor Surface Type (excluding 'hand')") +
...
```{r Donor_surface_type+porosity}
# 计算每个 Donor_surface_type 和 Donor_porosity 的唯一文章数
article_counts <- filtered_data_no_hand %>%
  group_by(Donor_surface_type, Gloves,Donor_porosity) %>%
  summarise(ArticleCount = n_distinct(id)) %>%
  ungroup()

# 绘制按文章数量分组的条形图，颜色根据 Donor_porosity 填充
ggplot(article_counts, aes(x = Donor_surface_type, y = ArticleCount, fill = Donor_porosity)) +
  geom_bar(stat = "identity") +
  xlab("Donor Surface Type") +
  ylab("Number of Articles") +
 # ggtitle("Number of Articles by Donor Surface Type (excluding 'hand')") +
...
# 使用 ggplot2 绘制图形
ggplot(y_data, aes(x = Relative_humidity * 100, y = Temperature, color = Organism_type)) +
  geom_point() +
  labs(
    x = "Relative Humidity (%)",  # 加入 RH 的单位
    y = "Temperature (°C)",       # 加入温度的单位
    color = "Organism type"       # 修改图例标题
  ) +
  theme_minimal() +
  scale_x_continuous(
    limits = c(0, 100),           # 确保 RH 范围为 0-100
    expand = c(0, 0)              # 移除多余的边界空白
  )
